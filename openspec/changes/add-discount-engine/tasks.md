...
- [x] 1.1 Create `app/src/discounts.ts` with the types from spec §5
...
- [x] 1.2 Implement internal helpers: `roundHalfUp`, category sum, and the
...
- [x] 2.1 Implement tier discount: `roundHalfUp(subtotal × tierPercent / 100)`
      from the full pre-discount subtotal (D-1, D-4); verify with AC-1 and
      AC-6 tests.
- [x] 2.2 Implement coupon application as a sequential fold in typed order
      with running remainder capping `min(nominal, base, remainder)`
      (D-3, D-5, D-7, D-13); verify with AC-3, AC-4, AC-9, AC-12 tests.
- [x] 2.3 Assemble `PriceBreakdown` (subtotal, tier, coupons,
...
- [x] 3.1 Create `app/src/discounts.test.ts` with tests `AC-1` … `AC-12`
...
- [x] 3.2 Add the two extra delta-spec scenarios: category coupon on an
...
- [x] 4.1 Run `cd app && npm test` and confirm 8 old + all new tests pass;
      confirm no changes to `types.ts` / `pricing.ts` via `git diff --stat`.
- [x] 4.2 Back-check against the spec: no behaviour in code without an AC, no
